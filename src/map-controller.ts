...
export class MapController {
  ...
  highlightRange(startIndex: number, endIndex: number): void {
    ...
    // 确保索引在有效范围内
    const safeStart = Math.max(0, Math.min(startIndex, visiblePoints.length - 1));
    const safeEnd = Math.max(0, Math.min(endIndex, visiblePoints.length - 1));

    const [s, e] = [Math.min(safeStart, safeEnd), Math.max(safeStart, safeEnd)];
    ...
    // 创建遮罩数据（未选中区域）
    const leftCoordinates = visiblePoints.slice(0, s + 1).map(p => [p.longitude, p.latitude, p.altitude]);
    const rightCoordinates = visiblePoints.slice(e).map(p => [p.longitude, p.latitude, p.altitude]);

    // 确保遮罩层存在
    if (!this.map.getSource('dim-left')) {
      // 左侧遮罩
    ...
    // 更新遮罩数据
    this._setSourceData('dim-left', {
      type: 'Feature',
      geometry: { type: 'LineString', coordinates: leftCoordinates },
      properties: {}
    });

    this._setSourceData('dim-right', {
      type: 'Feature',
      geometry: { type: 'LineString', coordinates: rightCoordinates },
      properties: {}
    });

    // 确保遮罩层在最上层显示
    if (this.map.getLayer('dim-left-line')) {
      this.map.moveLayer('dim-left-line', 'full-track-line');
      this.map.moveLayer('dim-right-line', 'dim-left-line');
    }
  }
  ...
  clearRangeHighlight(): void {
    ...
    if (this.map.getSource('dim-right')) {
      this._setSourceData('dim-right', {
        type: 'Feature',
        geometry: { type: 'LineString', coordinates: [] },
        properties: {}
